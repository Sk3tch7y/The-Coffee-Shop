...
{	// Load driver class
	...
	PreparedStatement customerName = con.prepareStatement("SELECT firstName, lastName, customerId FROM customer WHERE userid = '" + userId + "';");
	ResultSet nrst = customerName.executeQuery();
	String name = "";
	if(nrst.next() && nrst.getString("firstName") != null && nrst.getString("lastName") != null){
		name = nrst.getString("firstName") + " " + nrst.getString("lastName");
	}
	else{
		name = "No Name Given";
	}
	String custId = nrst.getString("customerId");
	PreparedStatement pstmt = con.prepareStatement("SELECT orderId, orderDate, customerId, totalAmount FROM ordersummary WHERE customerId = ? ORDER BY orderDate DESC;");
...
catch (java.lang.ClassNotFoundException e)
{
	...
	out.println("<h2 id = 'notFound'><a href = '/login.jsp'>log in</a></h2>");
}
